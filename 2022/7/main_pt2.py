...
class Directory:
    ...
    def GetOversizedDirectories(self, threshold):
        dirs = [d for d in self.Directories.values() if d.GetSize() >= threshold]
        [dirs.extend(d.GetOversizedDirectories(threshold)) for d in self.Directories.values()]
        return list(dirs)
...
spaceNeedeToRelease = spaceNeeded - (totalSpace - rootSize)

overSizedDirs = rootDir.GetOversizedDirectories(spaceNeedeToRelease)
overSizedDirs.sort(key=lambda d: d.GetSize())

print(overSizedDirs[0].GetSize())