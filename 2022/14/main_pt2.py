...
class Terrain:
    def __init__(self, rockLines, sandSpawnPoint):
        ...
        #add floor
        xOffset = 150
        pMin.X = pMin.X - xOffset
        pMax.Y = pMax.Y + 2
        pMax.X = pMax.X + xOffset
        floorStart = (pMin.X, pMax.Y)
        floorEnd = (pMax.X, pMax.Y)
        rockLines[0].append(Line((floorStart, floorEnd)))
    ...
    def TryMoveSand(self):
        sandAtRest = []
        sandHasReachedRestAtSpawnPoint = False
        for sand in self.ActiveSandList:
            for i in range(0, 99999): # 99999 is a fail safe
                ...
                if sand.Pos.X == self.SandSpawnPoint.X and sand.Pos.Y == self.SandSpawnPoint.Y:
                    sandHasReachedRestAtSpawnPoint = True
                break;
        ...
        return sandHasReachedRestAtSpawnPoint
...
sandSpawned = 0
sandReachedSpawnPoint = False
while not sandReachedSpawnPoint:
    terrain.TrySpawnSand()
    sandReachedSpawnPoint = terrain.TryMoveSand()
    sandSpawned += 1    

print('\n== FINAL STATE ==')
terrain.Print()
print(f'SandAtRest: {sandSpawned}')
